...
def post_student_list(request, payload: schemas.PostStudentListReq,):
    """
    {\n
        "name": "3-2반 명렬표",
        "description": "우리반 명렬표",
    ...
    # is_main == true 를 false로 바꿀 때
        ...
        try:
            recent_student_list = StudentList.objects.filter(
                user=user, is_main=False).order_by('-updated_at')[0]
        except ObjectDoesNotExist:
            raise ex.not_found_student_list
...
def delete_student_list(request, list_id: int):
    ...
    if student_list.is_main == True:
        try:
            recent_student_list = StudentList.objects.filter(
                user=request.auth, is_main=False).order_by('-updated_at')[0]
        except ObjectDoesNotExist:
            raise ex.not_found_student_list
        recent_student_list.is_main = True
        recent_student_list.save()
...
def put_student_list(request, payload: schemas.PutStudentListReq):
    ...
    with transaction.atomic():
        ...
        if not student_list.is_main and payload.is_main:
            try:
                main_student_list = StudentList.objects.get(
        ...
        for column in payload.columns:
            try:
                col = Column.objects.get(
                    id=column.id, student_list=student_list
                )
            except ObjectDoesNotExist:
        ...
        for student in payload.students:
            try:
                s = Student.objects.get(
                    id=student.id, student_list=student_list)
            except ObjectDoesNotExist:
